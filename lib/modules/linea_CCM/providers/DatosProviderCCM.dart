...
class ProviderCCM with ChangeNotifier {
  ...
  final RepoColorante = GenericRepositoryDatosList<ModeloColoranteCCM>(
    tableName: 'tablaColorante',
    fromMap: (map) => ModeloColoranteCCM.fromMap(map),
    toMap: (d) => d.toMap(),
    copyWithId: (d, id) => d.copyWith(id: id),
  );
  ...
  final RepoObservados = GenericRepositoryDatosList<ModeloObservados>(
   ...
   final RepoDatosCcmDef2 = GenericRepositoryDatosList<ModeloDatosCcmDef2>(
            tableName: 'tablaDatosCcmDef2',
            fromMap: (map) => ModeloDatosCcmDef2.fromMap(map),
            toMap: (d) => d.toMap(),
            copyWithId: (d, id) => d.copyWith(id: id),
          );
  ...
  Future<void> init() async {
    _db = await openDatabase(
      p.join(await getDatabasesPath(), 'tablalineaCCM.db'),
      version: 1,
  ...
  Future<void> createTable(Database db) async {
    ...
    ''');
     await db.execute('''
      CREATE TABLE ${RepoDatosCcmDef2.tableName} (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        hasErrors INTEGER NOT NULL,
        hasSend INTEGER NOT NULL,
        cod_dpcalidad INTEGER NOT NULL,
        ...
        Puntuacion_0 INTEGER NOT NULL,
        Puntuacion_1 INTEGER NOT NULL,
        Excentricidad INTEGER NOT NULL,
        DistIntMax INTEGER NOT NULL,
        DistExtMax INTEGER NOT NULL,
        Puntuacion_2 INTEGER NOT NULL,
        Puntuacion_3 INTEGER NOT NULL,
        AmplBandaBrill INTEGER NOT NULL
      )
    ''');
  ...
  Future<void> addColorante(ModeloColoranteCCM nuevoDato) async {
    await RepoColorante.add(_db, nuevoDato);
    notifyListeners();
  }
  ...
  Future<void> addDatosCcmDef2(ModeloDatosCcmDef2 nuevoDato) async {
    await  RepoDatosCcmDef2.add(_db, nuevoDato);  
    notifyListeners();
  }
  ...
  Future<void> updateColorante(int id, ModeloColoranteCCM updatedDato) async {
  ...
  Future<void> updateDatosCcmDef2(int id, ModeloDatosCcmDef2 updatedDato) async {
    await RepoDatosCcmDef2.update(_db, id, updatedDato);
  }
  ...
  Future<void> removeDatosCcmDef2(int id, void Function(VoidCallback onUndo) showUndoSnackBar) async {
   await RepoDatosCcmDef2.remove(_db, id, showUndoSnackBar, () => notifyListeners());
  }
  ...
  Future<bool> enviarDatosAPIPesos(int id) async {
    ...
    Future<bool> enviarDatosAPIDatosCcmDef2(int id) async {
    return await ApiService.enviarDatosBool(
      id: id,
      lista: RepoDatosCcmDef2.items,
      toJsonAPI: (d) => (d).toJsonAPI(),
      endpoint: Config().getEndpoint(1, 1),
    );
  }
